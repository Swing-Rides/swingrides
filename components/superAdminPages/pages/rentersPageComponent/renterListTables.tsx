...
import {
  AdminRenterData,
  AdminRenterDataRow,
  AdminRenterDataStatus,
  AdminVerificationPendingRenterData,
  AdminVerificationPendingRenterRow,
...
const normalizeRenterStatus = (status: string): AdminRenterDataStatus => {
...
const normalizeRenterRows = (
  rows: AdminRenterDataRow[],
): AdminRenterDataRow[] =>
  rows.map((row) => ({
    ...row,
    status: normalizeRenterStatus(String(row.status)),
  }));

const normalizeVerificationRows = (
  rows: AdminVerificationPendingRenterRow[],
): AdminVerificationPendingRenterRow[] =>
  rows.map((row) => ({
    ...row,
    status: normalizeRenterStatus(String(row.status)),
...
export const RentersListTable = ({
  data,
  pendingVerificationData,
}: RentersListTableProps) => {
  return (
    <>
      <Suspense>
      ...
      <div className="my-3 md:my-8 space-y-5">
        ...
        <div className="p-4 bg-amber-50 rounded border border-amber-100 flex justify-start items-center gap-3">
          ...
          <span className="text-amber-800 text-sm font-normal font-text leading-5">
            These renters have uploaded documents awaiting your review. High
            priority verification ensures a seamless rental experience.
          </span>
        </div>
      </div>
...
const TableSection = ({
  ...
  useEffect(() => {
    setRows(normalizeRenterRows(rowData));
  }, [rowData]);
    ...
    () =>
      rows.filter((r) => {
        const matchesStatus = statusFilter
          ? normalizeRenterStatus(String(r.status)) === statusFilter
          : true;
            ...
            {isLoading ? (
              ...
              paginated.map((item) => (
                <TableRow key={item.id}>
                  <TableCell>
                    <TableUIUser fullName={item.name} email={item.email} />
                  </TableCell>
                  <TableCell>
...
const VerificationTableSection = ({
  ...
  const [rows, setRows] = useState<AdminVerificationPendingRenterRow[]>(
    normalizeVerificationRows(rowData).filter((r) => r.status === "pending"),
  );
...
export const VerificationTableUIActionTab = ({
  onApprove,
  onReject,
}: {
  id: string;
...
export const ApproveDialogUI = ({
  pendingApproveId,
  setPendingApproveId,
  pendingRow,
  confirmApprove,
}: ApproveDialogUIProps) => (
  <AlertDialog
...
export const RejectDialogUI = ({
  ...
  <AlertDialog
    ...
    onOpenChange={(open) => {
      if (!open) setPendingRejectId(null);
    }}
  >
    <AlertDialogContent>
      <AlertDialogHeader>
