...
import { exportAllPaymentsToCSV, exportAllChangeHistoryToCSV } from '../../utils/exportToCSV'
...
type PaymentRow = IBIllingsResponse['data']['payments']['rows'][number]
type PlanChangeRow = IBIllingsResponse['data']['planChanges']['rows'][number]
...
// ─── CSV helpers moved to utils/exportBillingCSV.ts ─────────────────────────

// ─── Page ─────────────────────────────────────────────────────────────────────

export default function BillingPageComponents({ data }: { data: IBIllingsResponse['data'] }) {

        const { summaryCards, charts, payments, planChanges } = data

        // Map summary cards by key for easy access
        const cardMap = useMemo(
                () => Object.fromEntries(summaryCards.map((c) => [c.key, c])),
                [summaryCards]
        )
                        ...
                        <Tabs value={activeTab} onValueChange={handleTabChange}>
                                <div className='px-4 pt-4'>
                                        <TabsList variant='line'>
                                                ...
                                                <TabsTrigger value='plan_changes'>Upgrade / Downgrade History</TabsTrigger>
                                        </TabsList>
                                </div>

                                <TabsContent value='payments'>
                                        <Suspense>
                                                <AllPaymentTable rows={payments} />
                                        </Suspense>
                                </TabsContent>

                                <TabsContent value='plan_changes'>
                                        <Suspense>
                                                <PlanChangesTable rows={planChanges} />
                                        </Suspense>
                                </TabsContent>
                        </Tabs>
                </div>
...
function AllPaymentTable({ rows }: { rows: PaymentRow[] }) {
        const router = useRouter()
        const pathname = usePathname()
        const searchParams = useSearchParams()

        // Local search (instant, no URL)
        ...
        const page = Math.max(1, Number(searchParams.get('payment_page') ?? 1))

        const setPage = useCallback((p: number) => {
                const params = new URLSearchParams(searchParams.toString())
                params.set('payment_page', String(p))
                router.replace(`${pathname}?${params.toString()}`, { scroll: false })
        }, [router, pathname, searchParams])
        ...
        const filtered = useMemo(() => {
                ...
                return result
        }, [rows, search, planFilter, statusFilter, sortDir])
        ...
        return (
                <div className='p-4 md:p-6 flex flex-col gap-4.5'>
                        ...
                        <div className='flex flex-wrap gap-3 items-center'>
                                ...
                                <SelectUI title='All Plans' items={planItems} paramKey='plan' />
                                <SelectUI title='All Status' items={statusItems} paramKey='status' />

                                <button
                                        onClick={() => exportAllPaymentsToCSV(filtered)}
                        ...
                        <Table>
                                <TableHeader className='bg-gray-100'>
                                        <TableRow>
                                                <TableHead className="w-56 text-gray-500 text-xs font-semibold font-text uppercase leading-4">Organisation</TableHead>
                                                <TableHead className='text-gray-500 text-xs font-semibold font-text uppercase leading-4'>Plan</TableHead>
                                                <TableHead className='text-gray-500 text-xs font-semibold font-text uppercase leading-4'>Amount</TableHead>
                                                <TableHead className='text-gray-500 text-xs font-semibold font-text uppercase leading-4'>
                                                        ...
                                                        >
                                                                Billing Date
                                                                <CalendarArrowDown className={`size-3 transition-colors ${sortDir === 'asc' ? 'text-blue-600' : 'text-gray-400'}`} />
                                                                <CalendarArrowUp className={`size-3 transition-colors ${sortDir === 'desc' ? 'text-blue-600' : 'text-gray-400'}`} />
                                                        </button>
                                                </TableHead>
                                                <TableHead className='text-gray-500 text-xs font-semibold font-text uppercase leading-4'>Payment Method</TableHead>
                                                <TableHead className='text-gray-500 text-xs font-semibold font-text uppercase leading-4'>Status</TableHead>
                                                <TableHead className="w-20 text-gray-500 text-xs font-semibold font-text uppercase leading-4">Actions</TableHead>
                                        </TableRow>
                                </TableHeader>
                                <TableBody>
                                        {paginated.length === 0 ? (
                                                ...
                                                <TableRow key={item.id}>
                                                        ...
                                                        <TableCell>
                                                                <SubscriptionBillingStatusPill status={item.status.toLowerCase() as SubscriberBillingStatus} />
                                                        </TableCell>
                                                        <TableCell>
...
function PlanChangesTable({ rows }: { rows: PlanChangeRow[] }) {
        const router = useRouter()
        const pathname = usePathname()
        const searchParams = useSearchParams()
        ...
        const page = Math.max(1, Number(searchParams.get('changes_page') ?? 1))

        const setPage = useCallback((p: number) => {
                const params = new URLSearchParams(searchParams.toString())
                params.set('changes_page', String(p))
                router.replace(`${pathname}?${params.toString()}`, { scroll: false })
        }, [router, pathname, searchParams])
        ...
        return (
                <div className='p-4 md:p-6 flex flex-col gap-4.5'>
                        ...
                        <div className='flex flex-wrap gap-3 items-center'>
                                ...
                                <button
                                        onClick={() => exportAllChangeHistoryToCSV(filtered)}
                                        className='flex flex-nowrap text-nowrap gap-2 items-center px-3 py-2 rounded-[10px] border border-blue-700 cursor-pointer hover:bg-blue-900 duration-300 transition-colors group'
                                >
                                        <span className="text-blue-700 text-sm font-medium font-text leading-5 group-hover:text-blue-200 duration-300 transition-colors">
                                                Export CSV
                        ...
                        <Table>
                                <TableHeader className='bg-gray-100'>
                                        <TableRow>
                                                <TableHead className="w-56 text-gray-500 text-xs font-semibold font-text uppercase leading-4">Organisation</TableHead>
                                                <TableHead className='text-gray-500 text-xs font-semibold font-text uppercase leading-4'>Changed From</TableHead>
                                                <TableHead className='text-gray-500 text-xs font-semibold font-text uppercase leading-4'>Changed To</TableHead>
                                                <TableHead className='text-gray-500 text-xs font-semibold font-text uppercase leading-4'>Change Type</TableHead>
                                                <TableHead className='text-gray-500 text-xs font-semibold font-text uppercase leading-4'>Date</TableHead>
                                                <TableHead className='text-gray-500 text-xs font-semibold font-text uppercase leading-4'>Actioned By</TableHead>
                                        </TableRow>
                                </TableHeader>
                                <TableBody>
                                        {paginated.length === 0 ? (
                                                ...
                                                <TableRow key={item.id}>
                                                        ...
                                                        <TableCell>
                                                                <ChangeTypeCell changeType={item.changeType as 'upgrade' | 'downgrade'} />
                                                        </TableCell>
                                                        <TableCell>
                                                                <span className="text-gray-500 text-sm font-normal font-text leading-5">
                                                                        {item.date}
...
function PaginationBar({
        page, totalPages, label, onPageChange,
}: {
        page: number
        totalPages: number
        label: string
...
function ChangeTypeCell({ changeType }: { changeType: 'upgrade' | 'downgrade' }) {
        const isUpgrade = changeType.toLowerCase() === 'upgrade'
        return (
                <span className={`flex items-center gap-1.5 text-sm font-medium font-text leading-5 capitalize ${isUpgrade ? 'text-emerald-500' : 'text-red-500'}`}>
                        {isUpgrade
...
const OverviewCard = ({
        icon, title, presentDayNumber, last30Days, formatValue, invertColors = false,
}: OverviewCardProps) => {
        const { percentage, colorClass } = calcPercentageDiff(
                presentDayNumber, last30Days, { invertColors }
        )
