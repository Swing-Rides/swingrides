...
import { Activity, ChevronRight as BreadcrumbChevron, Calendar, ChevronLeft, ChevronRight, CircleCheckBig, CircleX, ExternalLink } from "lucide-react";
import { 
...
import { CarBookingStatusPill, RenterVerificationStatusPill } from "../../dashboard/statusPill";
...
export default function RenterPageComponents({ renter, stats, verification, documents, bookings }: RenterPageComponentsProps) {
        return (
...
const ProfileSideCard = ({ renter, stats, verification }: ProfileSideCardProps ) => {
        return (
                ...
                <div className="space-y-3 md:space-y-6">
                        <div className="flex flex-col gap-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                        <DataList 
                                                title="First Name"
                                                label={renter.firstName}
                                        />
                                        <DataList 
                                                title="Last Name"
                                                label={renter.lastName}
                                        />
                                        <DataList 
                                                title="Phone"
                                                label={renter.phone}
...
const BookingTable = ({ rowData, limit }: BookingTableProps ) => {
        ...
        const bookingId = '6a01a1ce39a46efd34fe0fe9'
                ...
                const paginated = rowData.slice((page - 1) * ROWS_PER_PAGE, page * ROWS_PER_PAGE)
                const goToPage = (p: number) => setParam('booking-table-page', String(p))
        
                const paginationLabel = rowData.length === 0
                        ? '0 results'
                                        ...
                                        <Suspense fallback={<TableSkeletonRows />}>
                                                {paginated.length === 0 ? (
                                                        ...
                                                        <TableRow key={item.bookingRef}>
                                                                ...
                                                                <TableCell>
                                                                        <Link href={`/admin/subscribers/${hostId}/bookings/${bookingId}`}>
...
const DocumentCard = ({ url, title, uploadedAt, status }: DocumentCardProps) => {
        return (
