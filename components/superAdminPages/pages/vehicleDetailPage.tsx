...
import { MaintenanceStatusPill, SubscriptionPlanPill } from "../dashboard/statusPill";
import Link from "next/link";
import { MaintenanceStatusType, SubscriberPlan } from "@/constants/superAdminSidebar";
                                                ...
                                                <div className="lg:col-span-1">
                                                        ...
                                                        >
                                                                <div className="grid gap-4">
                                                                        <DataList
                                                                                title="Total Trips"
                                                                                label={totalTrips ? totalTrips : "This vehicle is yet to complete any trip"}
                                                                                labelClassName={totalTrips ? "text-neutral-950 text-2xl font-normal font-text leading-8" : "text-red-500 text-xs font-normal font-text leading-5"}
                                                                        />
                                                                        <DataList
                                                                                title="Total Revenue"
                                                                                label={formatNumberToUSD(totalRevenue) ? totalRevenue : "This vehicle is yet to make a revenue"}
                                                                                labelClassName={totalRevenue ? "text-emerald-500 text-2xl font-normal font-text leading-8" : "text-red-500 text-xs font-normal font-text leading-5"}
                                                                        />
                                                                        <DataList
                                                                                title="Total Expenses"
                                                                                label={formatNumberToUSD(totalExpenses) ? totalExpenses : "This vehicle is yet to incur expense"}
                                                                                labelClassName={totalExpenses ? "text-red-500 text-2xl font-normal font-text leading-8" : "text-red-500 text-xs font-normal font-text leading-5"}
                                                                        />
                                                                        <DataList
                                                                                title="Net Earnings"
                                                                                label={formatNumberToUSD(netEarnings) ? netEarnings : "Not enough data to measure net earnings"}
                                                                                labelClassName={netEarnings ? "text-blue-700 text-2xl font-normal font-text leading-8" : "text-red-500 text-xs font-normal font-text leading-5"}
                                                                        />
                                                        ...
                                                        >
                                                                <div className="grid gap-4">
                                                                        ...
                                                                        <DataList 
                                                                                ...
                                                                                status={<MaintenanceStatusPill status={maintenanceStatus}/>}
                                                                        />
                                                                </div>
                                                        </SectionWrapper>
                                                        <SectionWrapper
...
const BaseDetailCard = ({ title, number }: BaseDetailCardProps ) => {
        ...
        return (
                <div className="basis-50 grow shrink flex-1 p-3 md:p-5 bg-white rounded-lg border border-gray-200 flex flex-col justify-start items-start gap-2">
                        ...
                        <span
                                className="text-neutral-950 text-2xl font-normal font-text leading-8"
                        >
                                {number}
                        </span>
                        <span>
...
export const InLineDataListPrice = ({ label, price, priceClassName }: { label: string; price: number; priceClassName?: string; }) => {
        return (
                <div className="flex justify-between items-center gap-3">
                        <span className="text-gray-500 text-sm font-normal font-text leading-5">
                                {label}
                        </span>
                        <span className={priceClassName ? priceClassName : `text-neutral-950 text-lg font-normal font-text leading-6`}>
