...
import React, { ReactNode, useCallback, Suspense, useState, useEffect, useMemo } from "react";
import Link from "next/link";
import { useRouter, useSearchParams, usePathname } from 'next/navigation'
import { ChevronLeft, ChevronRight, Eye, ChevronRight as BreadcrumbChevron, Trash } from "lucide-react";
import { Skeleton } from "@/components/ui/skeleton";
import { formatNumberToUSD } from "../utils/formatNumbertoUSD";
import { FLEET_STATUS_STYLE, BOOKING_STATUS_STYLE, BILLING_STATUS_STYLE, STATUS_STYLE } from "../utils/helpers";
...
import { AdminSubscriberByIdDataFleet,
        FleetStatus,
        BookingStatus,
        BillingStatus,
        BookingRow,
...
type SubscriberDetailPageProps = {
        ...
        plan: string
        joinedDate: string
        activityLog: { time: string; eventType: string; details: string; action: string }[]
}

// Discriminated union keeps DataTable fully type-safe per data type
type DataTableVariant =
        | { dataType: "Vehicle"; rows: AdminSubscriberByIdDataFleet[]; setRows: React.Dispatch<React.SetStateAction<AdminSubscriberByIdDataFleet[]>> }
        | { dataType: "Booking"; rows: BookingRow[]; setRows: React.Dispatch<React.SetStateAction<BookingRow[]>> }
        | { dataType: "Billing History"; rows: BillingRow[]; setRows: React.Dispatch<React.SetStateAction<BillingRow[]>> }
...
export default function SubscriberDetailPage({
        organisation, status, vehicles, activeBookings,
        monthlyRevenue, totalEarning, revenue, fleet, booking, billingHistory, slug,
}: SubscriberDetailPageProps & SlugType) {

        const revenueTrend: GraphDataType = {
                data: revenue.map((item) => ({ sales: item.totalRevenue, month: item.month })),
                series: [{ name: "sales", color: "#1A56DB" }],
        }

        // Each tab owns its own rows state so deletes are isolated
        const [fleetRows, setFleetRows] = useState<AdminSubscriberByIdDataFleet[]>(fleet ?? [])
        const [bookingRows, setBookingRows] = useState<BookingRow[]>(booking ?? [])
        const [billingRows, setBillingRows] = useState<BillingRow[]>(billingHistory ?? [])
        ...
        return (
                <div className='p-3 md:p-8'>
                        ...
                        <div className="flex flex-wrap gap-4 my-3 md:mt-10.5 md:mb-10">
                                <OverviewCard icon={<TotalVehiclesIcon />} title="Total Vehicles" number={vehicles} />
                                <OverviewCard icon={<ActiveBookingIcon />} title="Active Bookings" number={activeBookings} />
                                <OverviewCard icon={<MonthlyRevenueIcon />} title="Monthly Revenue (MRR)" number={formatNumberToUSD(monthlyRevenue)} />
                                <OverviewCard icon={<TotalRevenueIcon />} title="Total Revenue" number={formatNumberToUSD(totalEarning)} />
                        </div>
...
function DataTable(props: DataTableVariant & SlugType) {
        ...
        const paginationLabel = displayRows.length === 0
                ? '0 results'
...
function StatusBadge({
        ...
        const styleMap =
                dataType === "Vehicle" ? FLEET_STATUS_STYLE :
                        dataType === "Booking" ? BOOKING_STATUS_STYLE :
...
function TableUIActionTab({
        ...
        const href =
                dataType === "Vehicle" ? `/admin/subscribers/${slug}/fleet/${itemId}` :
                        dataType === "Booking" ? `/admin/subscribers/${slug}/bookings/${itemId}` :
                                `/admin/subscribers/${slug}/billing/${itemId}`

        return (
                <div className='flex gap-5 items-center'>
                        <Link href={href} className="text-[#6B7280] hover:text-blue-600 transition-colors">
...
export const PageIntro = ({ pageTitle, pageDesc, dataType = "subscribers", status }: PageIntroProps) => {

        const styleMap =
                dataType === "fleet" ? FLEET_STATUS_STYLE :
                        dataType === "booking" ? BOOKING_STATUS_STYLE :
                                STATUS_STYLE
        
        const normalizedStatus = status.toLowerCase() as SubscriberStatus | BookingStatus | FleetStatus | BillingStatus

        const { label, textColor, bgColor } = (styleMap as Record<string, { label: string; textColor: string; bgColor: string }>)[normalizedStatus]
                ?? (styleMap as Record<string, { label: string; textColor: string; bgColor: string }>)[normalizedStatus]
                ?? { label: status, textColor: '#6B7280', bgColor: '#F3F4F6' }
...
const DeleteDialogUI = ({ open, onOpenChange, name, dataType, onConfirm }: DeleteDialogUIProps) => (
...
const TotalVehiclesIcon = () => (
        <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M30.9911 28.9981H32.9906C33.5904 28.9981 33.9903 28.5982 33.9903 27.9984V24.9992C33.9903 24.0995 33.2905 23.2997 32.4907 23.0998C30.6912 22.5999 27.992 22.0001 27.992 22.0001C27.992 22.0001 26.6924 20.6005 25.7927 19.7008C25.2928 19.3009 24.693 19.001 23.9932 19.001H16.9952C16.3954 19.001 15.8955 19.4009 15.5956 19.9007L14.196 22.7999C14.0637 23.186 13.9961 23.5913 13.9961 23.9995V27.9984C13.9961 28.5982 14.396 28.9981 14.9958 28.9981H16.9952" stroke="#1A56DB" strokeWidth="1.99942" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M18.9936 30.9979C20.0978 30.9979 20.993 30.1027 20.993 28.9984C20.993 27.8942 20.0978 26.999 18.9936 26.999C17.8893 26.999 16.9941 27.8942 16.9941 28.9984C16.9941 30.1027 17.8893 30.9979 18.9936 30.9979Z" stroke="#1A56DB" strokeWidth="1.99942" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M20.9941 28.998H26.9924" stroke="#1A56DB" strokeWidth="1.99942" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M28.9916 30.9979C30.0959 30.9979 30.991 30.1027 30.991 28.9984C30.991 27.8942 30.0959 26.999 28.9916 26.999C27.8874 26.999 26.9922 27.8942 26.9922 28.9984C26.9922 30.1027 27.8874 30.9979 28.9916 30.9979Z" stroke="#1A56DB" strokeWidth="1.99942" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
)

const ActiveBookingIcon = () => (
        <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M0 9.99999C0 4.47714 4.47715 0 10 0H37.9997C43.5226 0 47.9997 4.47715 47.9997 10V37.9997C47.9997 43.5226 43.5226 47.9997 37.9997 47.9997H9.99999C4.47714 47.9997 0 43.5226 0 37.9997V9.99999Z" fill="#E3FAFF" />
                <path d="M19.998 14.002V18.0008" stroke="#0891B2" strokeWidth="1.99942" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M27.9961 14.002V18.0008" stroke="#0891B2" strokeWidth="1.99942" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M30.9953 16.002H16.9994C15.8952 16.002 15 16.8971 15 18.0014V31.9973C15 33.1015 15.8952 33.9967 16.9994 33.9967H30.9953C32.0996 33.9967 32.9948 33.1015 32.9948 31.9973V18.0014C32.9948 16.8971 32.0996 16.002 30.9953 16.002Z" stroke="#0891B2" strokeWidth="1.99942" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M15 22H32.9948" stroke="#0891B2" strokeWidth="1.99942" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
)
...
const TotalRevenueIcon = () => (
        <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M33.9942 19.001L25.4967 27.4985L20.4981 22.5L14 28.9981" stroke="#F59E0B" strokeWidth="1.99942" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M27.9961 19.001H33.9943V24.9992" stroke="#F59E0B" strokeWidth="1.99942" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
)