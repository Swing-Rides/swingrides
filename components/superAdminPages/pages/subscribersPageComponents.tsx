...
import { ChevronLeft, ChevronRight, Download, Eye, Search, Trash } from 'lucide-react'
import {
        Select,
        SelectContent,
        SelectGroup,
...
// import { type SubscriberStatus, type SubscriberPlan } from '@/constants/superAdminSidebar'
...
import { AdminSubscriberRow, AdminSubscribersListData, SubscriberPlan, SubscriberStatus } from '@/types/subscribers.type'
...
export default function SubscribersPageComponents({ subscriberData } : { subscriberData: AdminSubscribersListData}) {

        const { summary, rows } = subscriberData
        
        // const [rows, setRows] = useState<Subscriber[]>(subscribersTestData)
        ...
        return (
                ...
                >
                        <div>
                                ...
                                <Suspense>
                                        <SubscribersTableSection dataRows={rows}/>
                                </Suspense>
                        </div>
                </PageWrapper>
        )
...
function SubscribersTableSection({
        ...
        // ── Filters from URL ──────────────────────────────────────────────────
        const statusFilter = (searchParams.get('status') ?? '') as SubscriberStatus | ''
                                                ...
                                                <Suspense fallback={<TableSkeletonRows />}>
                                                        {paginated.length === 0 ? (
                                                                ...
                                                                <TableRow key={item.id}>
                                                                        ...
                                                                        <TableCell>
                                                                                <span className="text-sm text-neutral-800">{item.vehicles}</span>
                                                                        </TableCell>
                                                                        <TableCell>
                                                                        ...
                                                                        <TableCell>
                                                                                <span className="text-sm text-neutral-800">{formatDate(item.joinedDate)}</span>
                                                                        </TableCell>
                                                                        <TableCell>
                                                                                <TableUIActionTab
                                                                                        slug={item.id}
...
export const OverviewCard = ({ label, textColor, number }: OverviewCardProps) => (
        <div className='p-6 bg-white rounded-md border border-gray-200 flex flex-col gap-2 flex-1 basis-47.5 grow shrink-0'>
                ...
                <span className='text-3xl font-medium font-text' style={{ color: textColor }}>
                        {number}
                </span>
        </div>
)
...
const TableUIOrganisation = ({ organization, ownerEmail }: { organization: string; ownerEmail: string }) => {
...
export const TableUIActionTab = ({ slug, onDelete }: { slug: string; onDelete: () => void }) => (
