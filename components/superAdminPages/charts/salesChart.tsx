...
/**
 * Returns the latest `count` points from the provided series.
 * The backend already returns ordered monthly data, so we keep the UI aligned
 * with that sequence rather than inventing a calendar index.
 */
function getFilteredData(filter: FilterType, graphData: GraphDataType) {
  const { data } = graphData;
  const count = MONTH_COUNT[filter];

  return data.length <= count ? data : data.slice(-count);
}
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="flex flex-wrap gap-4 justify-between items-center">
        <CardIntro title="MRR Growth" desc="Monthly recurring revenue trend" />
