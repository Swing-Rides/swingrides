...
import { AlertCircle, AlertTriangle, Calendar, Check, CheckCircle2, Clock, CreditCard, Mail, MapPin, Phone, Shield, User, XCircle } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { ReactNode } from "react";
import { BookingSummaryInfo, CheckRecord, CheckType, DamageReportInfo, ExtraItem, IncidentalInfo, NoShowInfo, PaymentInfo, PreCheckStatusInfo, ReimbursementInfo, RenterInfo, TimelineEvent, TripInfo, VehicleInfo } from "./types";


export const RenterCard = ({ renterFullname, renterEmail, renterPhone, renterLicenseNumber }: RenterInfo) => {
        return (
...
export const VehicleCard = ({ vechicleId, vehicleImageSrc, vehicleName, vehicleType, vehiclePlateNumber, vehicleGearType }: VehicleInfo) => {
        return (
...
export const TripCard = ({ PickUpDateTime, ReturnDateTime, PickUpLocation, ReturnLocation }: TripInfo) => {
        return (
...
export const CheckInCheckOutCard = ({ checkIn, checkOut, bookingId }: CheckInCheckOutProps) => {
        return (
                <CardWrapper cardTitle="Check-In & Check-Out">
                        <div className="space-y-3">
                                {checkIn ? <CheckInCheckOutCompletedState
                                ...
                                {checkOut ? <CheckInCheckOutCompletedState
...
const CheckInCheckOutCompletedState = ({ checkedDateTime, bookingId, checkType, mileage, fuelLevel, condition, notes }: CheckRecord ) => {
        return (
                <div className = "p-4 bg-slate-200 rounded-[10px] flex flex-col gap-3">
                        <div className="flex items-center justify-between gap-4">
                                <div className="flex items-center justify-start gap-2">
                                        ...
                                        <DataListReverse
                                                label={checkType === "checkIn" ? "Check-In Completed" : "Check-Out Completed"}
                                                value={checkedDateTime}
                                        />
                                </div>
                                <div>
                                        <Link
                                                href={`${HOST_DASHBOARD_PATH}bookings/${bookingId}/${checkType === "checkIn"
                                                        ? "complete-check-in"
                                                        : "complete-check-out"
                                                }`}
                                                className="py-2 px-4 bg-transparent border border-blue-700 text-blue-700 rounded-xs hover:bg-blue-900 hover:text-white hover:border-blue-900 duration-300 transition-colors text-nowrap"
...
const CheckInCheckOutPendingState = ({ checkType, bookingId }: CheckInCheckOutPendingStateProps) => {
        return (
...
export const PreCheckStatusCard = ({ driverLicenseStatus, insuranceStatus, paymentStatus }: PreCheckStatusInfo) => {
        return (
...
const PreCheckStatusDataList = ({ icon, label, status }: PreCheckStatusDataListProps ) => {
        return (
...
export const DamageReportCard = ({ damageType, damageUserFullname, description, uploadedProof, isAcknowledged, onAcknowledge }: DamageReportCardProps ) => {
...
export const IncidentalCard = ({ incidentType, incidentDecription, incidentFee, incidentImages }: IncidentalInfo ) => {
        return (
                <CardWrapper cardTitle="Incidentals">
                        <div className="space-y-3 p-2 md:p-4 rounded-[10px] border border-gray-200">
                                <div className="flex items-start justify-between gap-4">
                                        <DataListReverse
                                                label={incidentType}
                                                value={incidentDecription}
                                        />
                                        <span className="jtext-red-500 text-base font-medium font-text leading-6">
                                                {incidentFee}
                                        </span>
                                </div>
                                <div className={`${incidentImages && incidentImages.length > 0 ? "grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3" : "flex justify-center items-center"}`}>
                                        {incidentImages && incidentImages.length > 0 ? incidentImages.map((src) => (
                                                <Image
                                                        key={src}
                                                        src={src}
                                                        alt="Incident Image"
...
export const ReimbursementCard = ({ reimbursementType, reimbursementDescription, reimbursementAmount, reimbursementRequestedDate, reimbursementProcessedDate, reimbursementStatus }: ReimbursementInfo ) => {
        return (
                        ...
                        <div className="flex flex-col md:flex-row gap-3 p-2 md:p-4 rounded-[10px] border border-gray-200">
                                <div className="flex flex-col gap-1.5">
                                        <div className="flex justify-start items-center gap-2">
                                                ...
                                                <span className={`${reimbursementStatus === "approved" ? "text-emerald-500 bg-emerald-100" : reimbursementStatus === "pending" ? "text-amber-500 bg-orange-100" : "text-red-500 bg-rose-100"} py-0.5 px-2 rounded-full inline-flex justify-center items-center text-xs font-semibold font-text leading-4`}>
                                                        {reimbursementStatus === "approved" ? "Approved" : reimbursementStatus === "pending" ? "Pending" : "Rejected"}
                                                </span>
                                        </div>
                                        ...
                                        <span className="block text-gray-500 text-xs font-normal font-text leading-4">
                                                Requested: {reimbursementRequestedDate} · Processed: {reimbursementProcessedDate || "Pending"}
                                        </span>
                                </div>
                                <div>
        ...
        return (
                <div className="flex items-start justify-start gap-4 rounded-lg border-l-4 border-red-500 p-3 md:py-6 md:px-8 bg-white">
                        ...
                        <div className="flex flex-col gap-2.5">
                                ...
                                <span className="text-gray-500 text-sm font-normal font-text leading-5">
                                        Pickup window expired on <span className="text-gray-800 font-semibold"> {pickUpDate} at {pickUpTime}</span>
                                </span>
                                <span className="text-gray-500 text-sm font-normal font-text leading-5">
                                        This booking has been automatically closed. No further actions are required unless incidentals apply.
                                </span>
                        </div>
                </div>
        )
...
export const BookingSummaryCard = ({ totalPaidByRenter, platformCommission, netToHost }: BookingSummaryInfo ) => {
        return (
...
export const PaymentStatusCard = ({ paymentStatus, totalPaidByRenter, paymentDate, paymentReciptSrc, refund, refundAmount, cancellationFeeAppliedDate }: PaymentInfo ) => {

        const statusColor = paymentStatus === "paid" ? "text-emerald-500 bg-emerald-100" : paymentStatus === "pending" ? "text-amber-500 bg-amber-100" : "text-red-500 bg-red-100";
        const statusText = paymentStatus === "paid" ? "Paid" : paymentStatus === "pending" ? "Pending" : "Failed";
        ...
        return (
                <CardWrapper cardTitle="Payment Status">
                        <div className="flex flex-col gap-4">
                                ...
                                />
                                {paymentStatus === "pending" ? (<div className="p-3 bg-amber-100 rounded-[10px] flex gap-1 justify-start items-start">
                                        <AlertCircle className="text-amber-500 size-4"/>
                                        <span className="block text-amber-500 text-xs font-normal font-text leading-4">Payment to be made at online reservation</span>
                                ...
                                {refund && <div className="p-3 bg-amber-100 rounded-[10px] border border-amber-200 flex flex-col justify-start items-start gap-1">
...
export const BookingTimeline = ({ bookingCreated, checkInCompleted, checkOutCompleted }: BookingTimelineProps) => {
        return (
...
export const CardWrapper = ({ children, cardTitle }: { children: React.ReactNode; cardTitle: string; }) => {
        return (
...
export const DataListReverse = ({ label, value, valueColor = "text-neutral-950" }: DataListProps ) => {
        return (
                <div className="flex flex-col gap-1 justify-start items-start">
                        <div>
                                <span className={`text-neutral-950 text-sm font-semibold font-text leading-5 ${valueColor}`}>
                                        {label}
                                </span>
                        </div>
                        <div className="flex justify-start items-center gap-2">
...
export const DataList = ({ icon, label, value, valueColor = "text-neutral-950" }: DataListProps ) => {
        return (
                <div className="flex flex-col gap-1 justify-start items-start">
                        <div className="flex justify-start items-center gap-2">
                                {icon && icon}
...
export const InlineDataList = ({ label, value, valueStyle = "text-gray-800 text-sm leading-5" }: InlineDataListProps ) => {
        return (
                <div className="flex justify-between items-center gap-3">
                        <span className="block text-gray-500 text-sm font-normal font-text leading-5">
                                {label}
...
export const BookingPageWrapper = ({ leftComponents, rightComponents }: BookingPageWrapperProps) => {
        return (
