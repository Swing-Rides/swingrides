...
import { useForm, Controller, useWatch, Control, FieldValues, Path, RegisterOptions } from 'react-hook-form'
...
const logServiceToDatabase = async (values: LogMaintenanceFormValues) => {
        ...
        // })
        console.log('logging maintenance service:', values)
}

// ─── Component ────────────────────────────────────────────────────────────────

export default function LogMaintenanceServiceForm({ onClose }: LogMaintenanceServiceFormProps) {
        ...
        const onSubmit = async (values: LogMaintenanceFormValues) => {
                await logServiceToDatabase({ ...values, nextServiceTab: activeTab })
                onClose()
        }
                                        ...
                                        {activeTab === 'mileage' ? (
                                                        ...
                                                        <Input
                                                                ...
                                                                {...register('nextServiceMileage', {
                                                                        ...
                                                                        validate: (value) => {
                                                                                ...
                                                                                const current = parseInt((mileageAtService ?? '').replace(/,/g, ''), 10)
                                                ...
                                                >
                                                                <DatePickerField<LogMaintenanceFormValues>
                                                                        ...
                                                                        error={errors.nextServiceDate?.message}
                                                                        rules={{
                                                                                required: activeTab === 'date' ? 'Next service date is required' : false,
