...
export default function CompleteRegistration() {
        ...
        // Tracks the most recent request so a stale response can't clobber a newer one.
        const requestIdRef = useRef(0)

        // (Re)creates the PaymentIntent. Called directly from the event handlers below
        // whenever the package, cycle, or coupon changes — not from an effect.
        const preparePayment = async (pkg: Package | null, cycle: BillingCycle, coupon: AppliedCoupon | null) => {
                if (!pkg) {
                ...
                const { total: amount } = computeTotals(pkg, cycle, coupon)
        ...
        const handleSelectPackage = (pkg: Package) => {
                ...
                setError(null)
                preparePayment(pkg, billingCycle, appliedCoupon)
        }

        const handleSelectBillingCycle = (cycle: BillingCycle) => {
                setBillingCycle(cycle)
                if (selectedPackage) preparePayment(selectedPackage, cycle, appliedCoupon)
        }
        ...
        const handleApplyCoupon = async () => {
                ...
                if (result.valid && result.label && result.percentOff !== undefined) {
                        ...
                        setCouponInput("")
                        if (selectedPackage) preparePayment(selectedPackage, billingCycle, coupon)
                } else {
        ...
        const handleRemoveCoupon = () => {
                ...
                setCouponError(null)
                if (selectedPackage) preparePayment(selectedPackage, billingCycle, null)
        }
