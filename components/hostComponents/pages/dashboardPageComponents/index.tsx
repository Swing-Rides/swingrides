...
import { Calendar, Car, Clock, DollarSign, TrendingDown, TrendingUp, Wrench } from "lucide-react";
...
const DashboardOverviewCard = ({ icon, iconBgColor, title, number, trend, trendText }: DashboardOverviewCardProps ) => {
        return (
                <div className="basis-62.5 shrink-0 grow p-4 md:p-6 bg-white rounded-md border border-gray-200 flex flex-col justify-start items-start gap-2">
                        ...
                        <div>
                                <div className="flex items-center gap-0.5">
                                        {trend ? <TrendingUp className="text-emerald-500 size-4" /> : <TrendingDown className="text-red-500 size-4" />}
                                        <span className={`text-xs font-semibold font-text ${trend ? 'text-emerald-500' : 'text-red-500'}`}>
                                                {trendText}
                                        </span>
                                </div>
                        </div>
...
const FleetDataList = ({ icon, iconBg="bg-green-100", label, number }: FleetDataListProps) => {
        return (
                <div className="flex itmes-center justify-between">
                        <div className="flex items-center justify-start gap-2">
                                <div className={`size-8 ${iconBg} rounded-[10px] flex justify-center items-center`}>
                                        {icon}
                                </div>
