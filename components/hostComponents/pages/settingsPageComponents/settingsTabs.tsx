...
import { SendAgreementForm, UploadDocumentForm } from '../../forms/agreementForms';
...
import ProfileCompanyForm, { ProfileCompanyFormValues } from '../../forms/profileCompanyForm';
...
export const ProfileCompanyTab = ({ loading, defaults, photoUrl, onSubmit }: ProfileCompanyTabProps) => {
        if (loading || !defaults) {
...
const ProfileCompanyFormSkeleton = () => (
        <div className='flex flex-col gap-6'>
                {Array.from({ length: 4 }, (_, i) => (
                        <div key={i} className='p-4 md:p-6 bg-white rounded-[10px] border border-gray-200 flex flex-col gap-6'>
                                ...
                                <div className="flex items-center justify-between gap-5">
                                        <div>
                                                <div className='flex gap-3 items-center'>
                                                        <h4 className='font-text font-bold text-[20px]'>
                                                                {planName}
                                                        </h4>
                                                        <BillingPlanStatus
                                                                status={status}
...
const AgreementsCard = ({ title, label, previewLink, shareLink }: AgreementsCardProps) => {
        const agreementType: AgreementType = title === 'Long-Term'
                ? 'long-term'
                : 'commercial-fleet'
        ...
        return (
                <div className='grow shrink-0 p-3 md:p-6 bg-white rounded-[10px] space-y-4'>
                        ...
                        <div className='flex items-center gap-4'>
                                {previewLink ? (
                                        <Link href={previewLink} className='w-full text-nowrap cursor-pointer' target='_blank'>
...
export const PopupWrapper = ({ onClose, popupTitle, popupDescription, children }: PopupWrapperProps) => {
        return (
                <div
                                ...
                                <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-5 pt-5 pb-4 flex flex-col gap-3">
                                        <div className="flex items-center justify-between">
                                                <div className="flex flex-col gap-2">
                                                        ...
                                                        {popupDescription && <span className="text-sm font-normal font-text text-gray-400 text-center block">{popupDescription}</span>}
                                                </div>
                                                <button
                                                        onClick={onClose}
