...
export default function Faq({ faqs = [], categories = [] }: FaqProps) {
  ...
  const tabs = useMemo(
    () => [
      { id: ALL_QUESTIONS, title: "All Questions", slug: ALL_QUESTIONS },
      ...(categories || []),
    ],
    [categories]
  );
  ...
  const filteredFaqs = useMemo(() => {
    const list = faqs || [];
    if (tab === ALL_QUESTIONS) return list.slice(0, 10);
    return list.filter((faq) => faq.categorySlug === tab);
  }, [faqs, tab]);
      ...
      <div className="col-span-1 rounded-2xl border border-gray-200 bg-white py-3 md:py-6 px-2.5 md:px-5 flex flex-col gap-2 md:gap-4.5 divide-y">
        {tabs.map((item) => (
          <button
            key={item.id}
            type="button"
            className={`text-left px-6 py-4 rounded-xs text-sm md:text-xl font-semibold font-text leading-6 cursor-pointer ${
              tab === item.slug
                ? "bg-indigo-50 text-blue-700"
                : "bg-white text-gray-500"
            } hover:bg-gray-100 hover:text-blue-700 transition-colors duration-300`}
          ...
          <>
            ...
            >
              {filteredFaqs.map((item) => (
                ...
                >
                  <AccordionTrigger className="text-gray-800 text-base font-medium font-text leading-6 cursor-pointer">
                    {item.question}
                  </AccordionTrigger>
                  <AccordionContent className="text-gray-500 text-base font-normal font-text leading-6">
                    {item.answer}
                  </AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
            {tab === ALL_QUESTIONS && (faqs?.length ?? 0) > 10 && (
              <p className="text-xs text-gray-400 px-4 pt-2">
                Showing 10 of {faqs?.length ?? 0} questions — select a category
                to see more.
              </p>
            )}
          </>
