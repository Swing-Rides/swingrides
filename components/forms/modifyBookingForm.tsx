 ...
 */
const computeTotal = (days: number, rate: RentalRate | undefined ): { breakdown: string; total: number } => {
        ...
        if (months > 0) { total += months * rate.monthly; parts.push(pluralize(months, 'mo')) }
        if (weeks > 0) { total += weeks * rate.weekly; parts.push(pluralize(weeks, 'wk')) }
        if (remDays > 0) { total += remDays * rate.daily; parts.push(pluralize(remDays, 'day')) }

        return { breakdown: parts.join(' + '), total }
}

export default function ModifyBookingForm ({ rental, onClose }: ModifyFormProps) {
        const today = useMemo(() => {
                const d = new Date()
                d.setHours(0, 0, 0, 0)
                return d
        ...
        const onSubmit = async (values: ModifyFormValues) => {
                ...
                console.log('modifying trip:', { rentId: rental.rentId, ...values })
                onClose()
        }
        ...
        return (
                <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-4' noValidate>
                        ...
                        <div className='grid grid-cols-2 gap-3'>
                                ...
                                <FormRow label='Return Date' htmlFor='returnDate' error={errors.returnDate?.message}>
                                        <DatePickerField
                                                ...
                                                minDate={pickupDate ? addDays(new Date(pickupDate), 1) : addDays(today, 1)}
                                                rules={{
                        ...
                        <FormRow label='Street' htmlFor='pickupStreet' error={errors.pickupStreet?.message}>
                        ...
                        <div className='p-4 bg-indigo-50 rounded-[10px] flex flex-col justify-start items-start gap-1'>
                                <div className='flex gap-3 justify-between items-center w-full'>
                                        ...
                                        <span className='flex text-blue-700 text-base font-medium font-text leading-6 text-nowrap'>
                                                {computed ? `New Total: ${formatCurrency(computed.total)}` : 'New Total: $—'}
                                        </span>
                                </div>
