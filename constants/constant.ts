export const SITE_URL = "https://swingrides.com"

export const SUPER_ADMIN_DASHBOARD_PATH = `/admin/`

export const HOST_DASHBOARD_PATH = `/us/host/`

export const DISCORD_COMMUNITY_LINK = `https://discord.gg/XZ2f559h5N`

export const HOST_REGISTRATION_PATH = `/host/register/`

export const DEFAULT_IMAGE_SRC = `/images/swingrides-default-img.webp`