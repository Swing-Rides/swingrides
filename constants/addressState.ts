...
// USA states only (includes DC). Values are the standard 2-letter USPS codes.
...
export function findUSState(stateParam: string): SelectOption | undefined {
  if (!stateParam) return undefined;
  const decoded = decodeURIComponent(stateParam).trim().toLowerCase();
